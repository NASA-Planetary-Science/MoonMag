...
def PlotMap(lonMap_deg, latMap_deg, plotData, title, fName, levels=None, cmap=None):
...
def healpixExample():
    """
    Demonstrate usage and test application of HEALpix evaluation of spherical harmonic data and
    compare to cylindrical evaluation.
    """
    ...
    PlotMap(lonMap_deg, latMap_deg, mpix, MakeTitle('HEALpix evaluation', descrip, pmax), 'figures/healpix.pdf', levels=levels, cmap=Color.cmap['asymDev'])
    PlotMap(lonMap_deg, latMap_deg, asymDevs, MakeTitle('Direct cylindrical evaluation', descrip, pmax), 'figures/direct.pdf', levels=levels, cmap=Color.cmap['asymDev'])
    PlotMap(lonMap_deg, latMap_deg, hpDiff[pix], MakeTitle('HEALpix $-$ direct', descrip, pmax), 'figures/healpixDiff.pdf', levels=None, cmap=Color.cmap['BmapDiv'])


if __name__ == '__main__':
    # Import features and settings from PlanetProfile for convenience
    from PlanetProfile.Plotting.MagPlots import SetMap
    from PlanetProfile.GetConfig import Color, FigLbl, FigSize, FigMisc
